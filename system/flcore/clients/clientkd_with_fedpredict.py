...
class clientKDWithFedPredict(Client):
    ...
    def test_metrics(self, m, t, T, global_model):
        ...
        try:
            ...
            combined_model = fedpredict_client_torch(local_model=self.model[m].student, global_model=global_model,
                                                     t=t, T=100, nt=nt, device=self.device, fc=self.fraction_of_classes[m],
            ...
            with torch.no_grad():
                for x, y in testloaderfull:
                    ...
                    if self.last_training_round == 0:
                        output = self.combined_model[m](x)
                    else:
                        out_student, rep_student, out_teacher, rep_teacher = self.model[m].forward_kd(x)
                        output = out_teacher
                    if torch.isnan(output).any() and len(output) == self.batch_size[m]:
                        output = self.last_not_nan[:len(y)]
                    elif torch.isnan(output).any():
                        output = torch.from_numpy(
                            np.array(self.last_not_nan.detach().cpu().numpy().tolist() * self.batch_size[m])[:len(y)]).to(
                            self.device)
                    else:
    ...
    def train_metrics(self, m, t):
        ...
        try:
            ...
            self.past_train_metrics = train_acc, train_loss, train_num, train_balanced_acc, train_micro_fscore, train_macro_fscore, train_weighted_fscore, \
            self.current_alpha[m]
