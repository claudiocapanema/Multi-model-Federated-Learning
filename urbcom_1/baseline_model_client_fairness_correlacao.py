...
# =====================================================
# EXPERIMENTO 1 — VALIDAÇÃO DE GAMMA (EFICIÊNCIA REAL)
# =====================================================
...
gamma_history = []
efficiency_real_history = []

# -------- EXPERIMENTO 1 --------

client_contribution = {cid: 0.0 for cid in range(NUM_CLIENTS)}
client_participation = {cid: 0 for cid in range(NUM_CLIENTS)}
client_time_total = {cid: 0.0 for cid in range(NUM_CLIENTS)}

gamma_history = []
efficiency_real_history = []

def compute_gamma_values():
...
def reset_experiment_state(train_loaders):
    ...
    global client_participation
    global client_time_total
    global gamma_history
    global efficiency_real_history
    ...
    for fold in range(NUM_FOLDS):
        ...
        for rnd in range(1, ROUNDS + 1):
            ...
            for cid in clients_cifar:
                ...
                # 🔥 NOVO
                client_time_total[cid] += train_time
            ...
            for model_name, model in global_models.items():
                ...
                delta = global_acc - prev_global_acc[model_name]

                if model_name == "cifar":
                    participants = clients_cifar
                else:
                    participants = clients_gtsrb

                for cid in participants:
                    client_contribution[cid] += delta / len(participants)
                ...
                #
                #     "gamma_efficiency_spearman": spearman,
                #     "gamma_efficiency_kendall": kendall,
                #
            ...
            # =====================================================
            # APÓS AVALIAR TODOS OS MODELOS (UMA VEZ POR RODADA)
            # =====================================================

            gamma_dict = compute_gamma_values()

            gamma = np.array([gamma_dict[cid] for cid in range(NUM_CLIENTS)])
            ...
            for cid in range(NUM_CLIENTS):
                total_time = client_time_total[cid]
                total_gain = client_contribution[cid]

                eff = total_gain / (total_time + eps)

                eff_real.append(eff)

            eff_real = np.array(eff_real)

            # normalização
            if gamma.sum() > 0:
                gamma = gamma / gamma.sum()

            if np.abs(eff_real).sum() > 0:
                eff_real = eff_real / (np.abs(eff_real).sum())

            from scipy.stats import spearmanr, kendalltau

            if np.std(gamma) > 0 and np.std(eff_real) > 0:
                spearman = spearmanr(gamma, eff_real).correlation
                kendall = kendalltau(gamma, eff_real).correlation
            else:
                spearman = 0.0
                kendall = 0.0

            gamma_history.append(gamma)
            efficiency_real_history.append(eff_real)

            print(f"📈 Gamma vs Efficiency(real) | Spearman: {spearman:.4f} | Kendall: {kendall:.4f}")
            ...
            for result in round_results:
                row_data = {
                    ...
                    "global_acc": result["global_acc"],

                    "clients_cifar": real_training_counter["cifar"],
                    "clients_gtsrb": real_training_counter["gtsrb"],

                    "clients_selected": result["clients_selected"],
                    "clients_selected_total": sum(real_training_counter.values()),
                ...
                directory = f"{RESULTS_DIR}/frac_{FRAC}/alpha_dirichlet_{DIRICHLET_ALPHA}/beta_{BETA}/"
                filename = f"{directory}baseline_{result['dataset']}.csv"
        ...
        for g, e in zip(gamma_history, efficiency_real_history):
            if np.std(g) > 0 and np.std(e) > 0:
                corr = spearmanr(g, e).correlation
                valid_corrs.append(corr)

        final_spearman = np.mean(valid_corrs) if len(valid_corrs) > 0 else 0.0

        print("\n==============================")
        print(f"📊 FINAL Spearman Gamma vs Efficiency(real): {final_spearman:.4f}")
        print("==============================")
