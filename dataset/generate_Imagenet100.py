...
num_clients = 40
alpha = 5.0
dir_path = "ImageNet100/" + "clients_" + str(num_clients) + "/alpha_" + str(alpha) + "/"
...
def generate_dataset(dir_path, num_clients, niid, balance, partition, alpha):
    ...
    if not os.path.exists(f'{dir_path}/rawdata/'):
        os.system(f'wget --directory-prefix {dir_path}/rawdata/ http://cs231n.stanford.edu/imagenet-100.zip')
        os.system(f'unzip {dir_path}/rawdata/imagenet-100.zip -d {dir_path}/rawdata/')
    else:
    ...
    trainset = ImageFolder_custom(root=dir_path+'rawdata/imagenet-100/train.X1/', transform=transform)
    testset = ImageFolder_custom(root=dir_path+'rawdata/imagenet-100/val.X/', transform=transform)
