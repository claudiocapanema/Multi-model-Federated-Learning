...
def bar_auc(df, base_dir, x_column, first, second, x_order, hue_order):
    ...
    fig.savefig(
        """{}solutions_{}_clients_bar_per_dataset_auc_std.png""".format(base_dir,
                                                                num_clients), bbox_inches='tight',
        dpi=400)
    fig.savefig(
        """{}solutions_{}_clients_bar_per_dataset_auc_std.svg""".format(base_dir,
                                                                num_clients), bbox_inches='tight',
        dpi=400)
...
def bar(df, base_dir, x_column, first, second, x_order, hue_order):
    ...
    plt.subplots_adjust(wspace=0.07, hspace=0.14)
    fig.savefig(
        """{}solutions_{}_clients_bar_per_dataset_std.png""".format(base_dir,
                                                             num_clients), bbox_inches='tight',
        dpi=400)
    fig.savefig(
        """{}solutions_{}_clients_bar_per_dataset_std.svg""".format(base_dir,
                                                             num_clients), bbox_inches='tight',
        dpi=400)

def line(df, base_dir, x_column, first, second, hue, datasets, ci=None, style=None):
    ...
    for i in range(2):
        ...
        for j in range(2):
            title = datasets[j]
            line_plot(df=df, base_dir=base_dir, ax=axs[j],
    ...
    # fig.suptitle("", fontsize=16)
        ...
        plt.subplots_adjust(wspace=0.07, hspace=0.14)
        fig.savefig(
            """{}solutions_{}_clients_line_{}_std.png""".format(base_dir,
                                                    num_clients, y_column), bbox_inches='tight',
            dpi=400)
        fig.savefig(
            """{}solutions_{}_clients_line_{}_std.svg""".format(base_dir,
                                                    num_clients, y_column), bbox_inches='tight',
            dpi=400)
...
if __name__ == "__main__":
    ...
    solutions = ["MultiFedSpeed", "FedFairMMFL", "MultiFedAvg"]
    ...
    df = df.groupby(["Solution", "Round (t)"]).apply(lambda e: aggregate_metrics(e)).reset_index()
    first = 'Balanced accuracy std'
    second = 'Loss std'
    ...
    plt.plot()
    line(df, base_dir, "Round (t)", first, second, "Solution", datasets, ci=None)
