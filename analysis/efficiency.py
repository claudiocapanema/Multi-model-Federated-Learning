...
def read_data(read_solutions, read_dataset_order):
    ...
    solution_strategy_version = {
        ...
        "MultiFedEfficiency": {"Strategy": "MutlFedEfficiency", "Version": "Original", "Table": "MultiFedEfficiency"}}
    hue_order = []
    ...
    for solution in read_solutions:
        ...
        for i in range(len(paths)):
            ...
            try:
                ...
                df["Round efficiency"] = np.array((df[metric] / df["# training clients"]).tolist())
                df["Cumulative efficiency"] = np.array((df[metric].cumsum() / df["# training clients"].cumsum()).tolist())
...
if __name__ == "__main__":
    ...
    dataset = ["ImageNet_v2", "ImageNet"]
    dataset_alpha = {"ImageNet": 0.1, "ImageNet_v2": 10.0}
    ...
    tw = {"MultiFedEfficiency": "_tw_16", "MultiFedAvg": ""}
    ...
    # line(df, write_path, x="Round (t)", y="Accuracy (%)", hue="Strategy", style="Version", hue_order=hue_order)
    line(df, write_path, x="Round (t)", y="Round efficiency", hue="Strategy", y_max=20)
    line(df, write_path, x="Round (t)", y="Round efficiency", hue="Strategy", y_max=20)
    line(df, write_path, x="Round (t)", y="Cumulative efficiency", hue="Strategy", y_max=20)
    line(df, write_path, x="Round (t)", y="Cumulative efficiency", hue="Strategy", y_max=20)
